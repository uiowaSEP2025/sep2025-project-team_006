...
export default function ExcelViewer({ document_id }: ExcelViewerProps) {
    ...
    useEffect(() => {
        const fetchExcel = async () => {
            try {
                const url = webService.APPLICATION_DOCUMENT_GET.replace(":id", document_id);
                ...
                const isExcel = blobType === "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"
                if (!isExcel) {
