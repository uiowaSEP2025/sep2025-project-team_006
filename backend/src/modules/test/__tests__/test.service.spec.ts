...
import { Test as TestEntity } from '../../../entity/test.entity';
...
describe('TestService', () => {
    ...
    it('should return all test data', async () => {
        const testData = [{ id: 1, message: 'Hello', createdAt: new Date(), updatedAt: new Date() }];
    ...
    it('should create a new test entry', async () => {
        ...
        const savedTest = { id: 1, message: 'New Message', createdAt: new Date(), updatedAt: new Date() };

        jest.spyOn(repository, 'create').mockReturnValue(savedTest as any);
        jest.spyOn(repository, 'save').mockResolvedValue(savedTest);

        expect(await service.postTestData(newTest)).toEqual({ success: true, newEntry: savedTest });
    });

    it('should update a test entry', async () => {
        const existingTest = { id: 1, message: 'Old Message', createdAt: new Date(), updatedAt: new Date() };
        const updatedTest = { id: 1, message: 'Updated Message', createdAt: new Date(), updatedAt: new Date() };

        jest.spyOn(repository, 'findOne').mockResolvedValue(existingTest);
        jest.spyOn(repository, 'save').mockResolvedValue(updatedTest);

        expect(await service.putTestData('1', { message: 'Updated Message' })).toEqual({ success: true, updatedEntry: updatedTest });
    });

    it('should return error if updating non-existent entry', async () => {
        jest.spyOn(repository, 'findOne').mockResolvedValue(null);

        expect(await service.putTestData('99', { message: 'Updated Message' })).toEqual({
            success: false,
            error: 'ID not found',
        });
    });
